...
class DroneWrapper {
  constructor (id = 'SEARCHANDRESCUE_TEST', simulated = true, willDetect = false, location = { x: 0, y: 0 }, timeout = 1000, deployObjective = false, objectiveRadius = 30, moveSpeed = 10) {
    ...
    this.counter = 0
    this.newmysterycounter = 0
    this.simulated = simulated
    this.willDetect = willDetect
    this.timeout = timeout
    this.nextAction = this.init(deployObjective, simulated, location)
  }
  ...
  async listenForActions () {
    ...
    try {
      if (this.done === false) {
        console.log('Not done, no double spending.')
        setTimeout(fun, this.timeout)

        return null
      } else if (this.done === 'DETECTED') {
        return null
      } else if (this.done === 'FAILURE') {
        await this.drone.retrieveAsset().then((err, res) => {
          console.log('@retrieveAsset doit', this.drone.dbid, err, res)
        })
        this.done = true
        setTimeout(fun, this.timeout)
        return null
      ...
      if (detected) {
        ...
        if (this.drone.action === 'EXPLORE') {
          ...
          if (objectiveDetected) return null
      ...
      if (error.message === 'HTTP Error: Requested page not reachable') {
        this.done = 'FAILURE'
        return setTimeout(fun, this.timeout)
      }
      ...
      console.log(chalk.red(`too much error, bugging out, ${this.drone.dbid} bowing out`), error)
