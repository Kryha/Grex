...
class Drone {
  constructor (id, initLocation, fake = false) {
    ...
    this.keypair = orm.createKeyPair()
    this.dbid = null
  ...
  async _createDroneBigchain () {
    try {
      ...
      let drone = await orm.create(this.keypair, data, 'droneModel')
      this.dbid = drone.id
      this.bdbDrone = drone
  ...
  async retrieveAsset () {
    try {
      let updatedDrone = await orm.retrieve(this.dbid, 'droneModel')
  ...
  async setStateBigchain () {
    try {
      ...
      let updatedDrone = await orm.append(this.bdbDrone, this.keypair, data, this.dbid)
